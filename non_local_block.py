...
class NonLocalBlock(nn.Module):
    def __init__(self, in_channel, p=4, k=4, temperature=1e-3, device='cuda'):
    ...
    def forward(self, target, ref, ref_align):
        ...
        p_height = H // self.block_size  # h
        p_width = W // self.block_size  # w
        N = p_height * p_width
        ...
        # b, c, H, W -> b, 1, h, w, c -> b, 1, h/p, w, c, p -> b, 1, h/p, w/p, c, p, p
        # -> b, hw, c, p^2
...
if __name__ == '__main__':
    ...
    img1 = Image.open("00000005.png").convert("RGB")
    img2 = Image.open("00000006.png").convert("RGB")
    # img3 = Image.open("00000003.png").convert("RGB")
    # img4 = Image.open("00000004.png").convert("RGB")

    # i, j, h, w = RandomCrop.get_params(img1, output_size=(256, 256))
    # #
    ...
    img1 = torch.unsqueeze(ToTensor()(img1), dim=0).cuda()
    img2 = torch.unsqueeze(ToTensor()(img2), dim=0).cuda()
    ...
    model1 = NonLocalBlock(3, p=10, k=4, temperature=1e-3).cuda()
    model2 = NonLocal(3, 3)
    output = model1(img2, img1, img1)
    # output = model2(in2, in1, in1)
    img = ToPILImage()(output[0])
    print(torch.sum(output - img1))
    img.save('save' + str(0) + '.jpg')
    # for b in range(2):
